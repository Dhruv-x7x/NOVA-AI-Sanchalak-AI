...
class TemplateEngine:
    ...
    def _get_common_css(self) -> str:
        ...
        td { padding: 12px 15px !important; border-bottom: 1px solid #e2e8f0 !important; font-size: 14px !important; color: #000000 !important; font-weight: 600 !important; }
        
        /* Specific visibility fix for action items and strategic tables */
        .section table td, .section table td strong { color: #000000 !important; }
    ...
    def _get_inline_template(self, template_id: str) -> str:
        # Logo embedded as base64 (48x48 a6on-i logo)
        ...
        shell_start = """<html><head><meta charset="UTF-8"><style>{{ common_css }}</style></head><body><div id="a6on-i-report-root"><div class="classification-banner">CONFIDENTIAL - FOR INTERNAL USE ONLY</div><div class="container">"""

        shell_end = """<div class="footer"><div style="display:flex;align-items:center;gap:8px;"><img src="data:image/png;base64,""" + _LOGO_B64 + """" style="width:20px;height:20px;" />a6on-i | Generated by {{ _metadata.generated_by }}</div><div>Version {{ _metadata.version }} | Page 1 of 1</div></div></div></div></body></html>"""
    ...
    def _get_inline_text_template(self, template_id: str) -> str:
        h = "="*80 + "\n a6on-i - {{ _metadata.title | upper }}\n" + "="*80 + "\nID: {{ _metadata.report_id }} | Generated: {{ _metadata.generated_at }}\n" + "-"*80 + "\n"
        f = "\n" + "-"*80 + "\n(c) 2026 a6on-i | Version 1.1 | CONFIDENTIAL\n" + "="*80
